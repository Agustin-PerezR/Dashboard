...
_Generado automáticamente el 2026-09-04T11:04:48.865Z -- no editar a mano, se sobreescribe en cada publicación._
...
## 5.5. Mover el estado del Requerimiento (rol `developer`)

El tablero no se mueve solo en todos los proyectos: el webhook de GitHub/GitLab es
opcional y muchos repos no lo tienen configurado. Si estás trabajando un
Requerimiento, movelo vos con dos PATCH, uno al arrancar y otro al terminar.
...
Deja la tarjeta en **Haciendo**. Si el Requerimiento estaba sin asignar, ese mismo
PATCH te lo asigna a vos (la cuenta dueña de la key) y arranca el cómputo de tiempo
real -- no hace falta ningún paso previo. Si ya lo tiene otra persona, contesta
`403 "Este Requerimiento no está asignado a vos: ..."`: no es la key ni el endpoint,
así que no los cambies -- esa tarjeta hay que hablarla, no insistirla. El mismo 403
sale para cualquier otro PATCH sobre algo que no es tuyo: sólo `"status":"doing"` te
lo asigna solo.

**Al terminar la implementación, o al abrir el Pull Request:**

```bash
cat > /tmp/cuerpo.json <<'JSON'
{
  "status": "pr_open",
  "observations": "Implementación completada y verificada"
}
JSON
...
Deja la tarjeta en **Hecho / Dev** y congela el reloj. Poné en `observations` lo que
realmente se hizo: ese texto es lo que lee quien revisa.

Un developer sólo puede fijar a mano `to_do`, `doing` y `pr_open`. `merged_dev`,
`in_testing`, `tested` e `in_production` son consecuencia de un merge o de una
promoción y devuelven `400` si los mandás vos: ponerlos a mano haría que el tablero
afirme algo que el repositorio no respalda.

## 5.6. Integrar a dev lo que ya pidió el merge (roles `scrum_master` y `project_manager`)

Es la contracara del paso anterior. El developer deja la tarjeta en `pr_open`
(**⏳ PR abierto**) y ahí se queda: significa "de mi lado está listo, falta que lo
mergeen". Quien coordina mergea la rama a `dev` y la tarjeta pasa a `merged_dev`
(**✓ dev**), sin cambiar de columna -- las dos viven en "Hecho / Dev".

**Antes de tocar nada, fijate si el repo tiene `.github/workflows/kanban-sync.yml`**
(o el webhook de GitLab configurado). Si lo tiene, el merge a `dev` ya mueve la
tarjeta solo y no hay que hacer ningún PATCH: hacerlo a mano no rompe nada, pero es
trabajo de más. Este paso es para los repos que no lo tienen, donde si no lo hacés
vos la tarjeta se queda en "PR abierto" para siempre.
...
El cuerpo va con `status` y nada más. A diferencia del developer, el `scrum_master`
**no** escribe `observations` (coordina, no reporta lo que hizo la tarjeta): si lo
agregás al cuerpo la API contesta `403 No podés editar estos campos del
Requerimiento: observations`, y no es la key ni el endpoint. El `project_manager` sí
puede mandarlo.

**Mandalo recién después de que el merge entró de verdad**, nunca antes: `merged_dev`
afirma que el código ya está en `dev`, y el tablero deja de coincidir con el
repositorio si se adelanta. Si el merge falla o se revierte, no hay evento que lo
deshaga solo.

Lo mismo se hace desde la web sin escribir ningún curl: la tarjeta en "Hecho / Dev"
trae el botón **"✓ Marcar mergeado a dev"** para estos dos roles.

Con la key de estos roles la API acepta cualquiera de los ocho estados (la
restricción de la sección 5.5 es sólo del `developer`), pero las etapas que siguen
tienen su propio dueño: `in_testing` y `tested` salen de QA, `in_production` del
merge a la rama de producción. Fijalos a mano sólo si este proyecto no tiene webhook.
...
- **`scrum_master`** → no tiene skill propio: coordina desde la web. Lo único que
  hace desde acá es integrar a `dev` lo que ya pidió el merge, con el PATCH de la
  sección 5.6.
