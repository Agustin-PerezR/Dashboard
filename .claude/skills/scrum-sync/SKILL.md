...
{"status":"pr_open","observations":"<qué se encontró implementado y dónde (sourceRef)>"}
JSON
...
- No fuerces `"status":"pr_open"` si todavía no abriste el Pull Request. Si sólo
  encontraste evidencia parcial, dejá el `status` afuera del PATCH (no lo toques) y volcá
  el detalle en `observations` nomás.
...
- Nunca fuerces `status` a `pr_open` sólo porque encontraste documentación que lo describe
